...
const PricingCard = ({
  ...
  return (
    <div
      id="calculator"
      class={twMerge(
        'flex flex-col items-start justify-start bg-white/10 px-4 py-5',
        className
      )}
    >
      <p class="text-[20px] leading-[24px] text-white md:text-[32px] md:leading-[40px]">
        ≈{formatPrice(total())}
      </p>
      <p class="mt-2 text-[12px] leading-[16px] text-[#6F6F6F]">
        The price might change after the interview
      </p>
      <ul class="mt-4 w-full space-y-3">
        <For each={items()}>
          {(item, index) => (
            <li
              class="flex w-full flex-row items-center justify-between gap-2 text-start"
            ...
            >
              ...
              >
                <span
                  class="flex h-4 w-4 flex-row items-center justify-center rounded border-2 border-white md:h-5 md:w-5"
                ...
                <p
                  class="text-start text-[12px] leading-[20px] text-white md:text-[16px] md:leading-[24px]"
                  style={{ opacity: selectedIds().includes(item.id) ? 1 : 0.5 }}
                >
                  {item.name}
                </p>
              </button>
              <p class="text-[12px] leading-[20px] text-[#6F6F6F] md:text-[16px] md:leading-[24px]">
      ...
      <div class="my-5 h-[1px] w-full bg-[#303030]" />
      <p class="text-[12px] leading-[20px] text-[#6F6F6F] md:text-[16px] md:leading-[24px]">
        Money-back guarantee. Wire transfer, crypto, or PayPal are accepted.
      </p>
      <ContactField
        buttonText="Preorder"
        placeholderText="Your telegram or email"
      />
    </div>
  );
};
